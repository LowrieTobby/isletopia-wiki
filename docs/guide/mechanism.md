...
使用`/channel`可查看当前所在频道以及成员。

使用`/channel 频道名称`即可加入该聊天频道，可用的聊天频道如下：黑, 深蓝, 深绿, 湖蓝, 深红, 紫, 金, 灰, 深灰, 蓝, 绿, 天蓝, 红, 粉红, 黄, 白。

其中"白"是默认频道，所有玩家默认所在的频道，该频道的消息会被转发到QQ群。

加入聊天频道后，聊天字体会发生变化，并且玩家只能看到自己所在频道的消息。

你可以使用`/channel`指定多个聊天频道。例如：`/channel 红 白`，这样你的消息会被同时发送到两个频道，并且你也能看到两个频道的消息。
...
使用"书"物品，左右键选择两点确定一个长方体，输入`/blueprint`即可获取此长方体内方块的蓝图。

将物品放在副手，蓝图放在左手，按下F即可为蓝图填充材料。

将填充材料后的蓝图右键某方块，弹出菜单，选择构建，如果没有方块冲突，即可立刻建造该蓝图保存的方块。如果蓝图缺失一些材料，这些材料会被跳过。推荐在构建前使用预览功能。